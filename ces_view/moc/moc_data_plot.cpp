/****************************************************************************
** Meta object code from reading C++ file 'data_plot.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.2.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../data_plot.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'data_plot.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.2.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_DataPlot_t {
    QByteArrayData data[9];
    char stringdata[91];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    offsetof(qt_meta_stringdata_DataPlot_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData) \
    )
static const qt_meta_stringdata_DataPlot_t qt_meta_stringdata_DataPlot = {
    {
QT_MOC_LITERAL(0, 0, 8),
QT_MOC_LITERAL(1, 9, 16),
QT_MOC_LITERAL(2, 26, 0),
QT_MOC_LITERAL(3, 27, 8),
QT_MOC_LITERAL(4, 36, 11),
QT_MOC_LITERAL(5, 48, 6),
QT_MOC_LITERAL(6, 55, 16),
QT_MOC_LITERAL(7, 72, 8),
QT_MOC_LITERAL(8, 81, 8)
    },
    "DataPlot\0setTimerInterval\0\0interval\0"
    "slotAddData\0data_x\0slotAddDataRange\0"
    "double[]\0data_len\0"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_DataPlot[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    1,   29,    2, 0x0a,
       4,    1,   32,    2, 0x0a,
       6,    2,   35,    2, 0x0a,

 // slots: parameters
    QMetaType::Void, QMetaType::Double,    3,
    QMetaType::Void, QMetaType::Double,    5,
    QMetaType::Void, 0x80000000 | 7, QMetaType::Int,    5,    8,

       0        // eod
};

void DataPlot::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        DataPlot *_t = static_cast<DataPlot *>(_o);
        switch (_id) {
        case 0: _t->setTimerInterval((*reinterpret_cast< double(*)>(_a[1]))); break;
        case 1: _t->slotAddData((*reinterpret_cast< double(*)>(_a[1]))); break;
        case 2: _t->slotAddDataRange((*reinterpret_cast< double(*)[]>(_a[1])),(*reinterpret_cast< int(*)>(_a[2]))); break;
        default: ;
        }
    }
}

const QMetaObject DataPlot::staticMetaObject = {
    { &QwtPlot::staticMetaObject, qt_meta_stringdata_DataPlot.data,
      qt_meta_data_DataPlot,  qt_static_metacall, 0, 0}
};


const QMetaObject *DataPlot::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *DataPlot::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_DataPlot.stringdata))
        return static_cast<void*>(const_cast< DataPlot*>(this));
    return QwtPlot::qt_metacast(_clname);
}

int DataPlot::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QwtPlot::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 3)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 3;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
